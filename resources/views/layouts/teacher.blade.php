    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-full">
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="mt-4 px-4 space-y-2 text-sm font-medium flex-1">
                <a href="{{ route('teacher.teacher.dashboard') }}"
                    class="flex items-center gap-3 py-2.5 px-4 rounded-lg transition {{ request()->routeIs('teacher.teacher.dashboard') ? 'bg-purple-100 text-purple-700 font-semibold' : 'hover:bg-gray-100' }}">
            ...
            <header class="flex items-center justify-between p-4 bg-white border-b h-16">
                ...
                <div class="flex items-center ml-auto">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                class="inline-flex items-center gap-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:text-gray-900 focus:outline-none transition ease-in-out duration-150">
