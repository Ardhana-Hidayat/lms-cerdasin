            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                @php
                    $menuClass = $selectedClass
                        ...
                        : 'group flex flex-col items-center justify-center p-6 bg-gray-100 rounded-xl border cursor-not-allowed opacity-60';
                    $menuTag = $selectedClass ? 'a' : 'div';
                @endphp

                <{{ $menuTag }} @if ($selectedClass) href="#" @endif class="{{ $menuClass }}">
                    <div class="text-3xl mb-2 {{ $selectedClass ? 'text-purple-600' : '' }}"> <i
                            class="fa-solid fa-book-open"></i> </div>
                    <h3
                        class="text-lg font-semibold {{ $selectedClass ? 'text-gray-800 group-hover:text-purple-600' : '' }}">
                        Materi</h3>
