...
@section('content')
    <div class="max-w-5xl bg-white shadow rounded-xl p-6">
        ...
        <form action="{{ route('teacher.materials.update', $material->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <!-- Konten -->
            <div class="col-span-2">
                <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Materi</label>
                <textarea name="material" id="material" rows="5"
                    class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400"
                    placeholder="Tuliskan isi atau ringkasan materi...">{{ old('material', $material->material) }}</textarea>
                @error('material')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($material->thumbnail)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $material->thumbnail) }}" alt="Thumbnail"
                            class="w-32 h-32 object-cover rounded-lg border shadow-sm">
                    </div>
                @endif
                <input type="file" name="thumbnail" id="thumbnail" accept="image/*"
                    class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400">
                <p class="text-xs text-gray-500 mt-1">Format: JPG, PNG, atau JPEG.</p>
                @error('thumbnail')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                <select name="classroom_id" id="classroom_id"
                    ...
                    @foreach ($classes as $class)
                        ...
                        <option value="{{ $class->id }}"
                            {{ old('classroom_id', $material->classroom_id) == $class->id ? 'selected' : '' }}>
                            {{ $class->name }}
                        </option>
                    @endforeach
                </select>
                ...
                @if ($material->file_path)
                    <div class="mb-2">
                        <a href="{{ asset('storage/' . $material->file_path) }}" target="_blank"
                            ...
                            <i class="fa-solid fa-file mr-1"></i> Lihat File Saat Ini
                        </a>
                    </div>
                @endif
                <input type="file" name="file_path" id="file_path"
                    class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400">
                <p class="text-xs text-gray-500 mt-1">Format: PDF, DOCX, PPTX, dll.</p>
                @error('file_path')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
