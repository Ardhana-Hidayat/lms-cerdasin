...
<div class="max-w-5xl bg-white shadow rounded-xl p-6">
    ...
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-purple-700">Tambah Pertanyaan</h1>
    ...
    {{-- Form --}}
    <form action="{{ route('teacher.quizzes.questions.store', $quiz->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @csrf
        ...
        <div class="col-span-2">
            ...
            <textarea name="question" id="question" rows="3"
                ...
                placeholder="Masukkan teks pertanyaan..." required>{{ old('question') }}</textarea>
            @error('question')
        ...
        <div>
            ...
            <input type="text" name="option_a" id="option_a"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400"
                placeholder="Masukkan opsi A" value="{{ old('option_a') }}" required>
            @error('option_a')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="option_b" id="option_b"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400"
                placeholder="Masukkan opsi B" value="{{ old('option_b') }}" required>
            @error('option_b')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="option_c" id="option_c"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400"
                placeholder="Masukkan opsi C" value="{{ old('option_c') }}" required>
            @error('option_c')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="option_d" id="option_d"
                class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-purple-400"
                placeholder="Masukkan opsi D" value="{{ old('option_d') }}" required>
            @error('option_d')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
